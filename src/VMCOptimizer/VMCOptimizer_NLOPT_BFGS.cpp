...
#include "VMCOptimizer_NLOPT_BFGS.h"
#include "system.h"
...
VMCOptimizer_NLOPT_BFGS::VMCOptimizer_NLOPT_BFGS(
     const runConfig& cfg,
    MCEngine& engine,
...
std::vector<double> VMCOptimizer_NLOPT_BFGS::optimize(std::vector<double> params, const std::vector<bool>& optimize_mask) {
    // nlopt setup
...
double VMCOptimizer_NLOPT_BFGS::nloptObjective(
    const std::vector<double>& params, std::vector<double>& grad, void* data
) {
    return static_cast<VMCOptimizer_NLOPT_BFGS*>(data)->computeMC(params, grad);
}