    ...
    std::vector<std::unique_ptr<class Particle>>& particles) 
{
    ...
    auto grad1 = torch::autograd::grad(
        { log_psi }, { pos },
        /*grad_outputs=*/{ torch::ones_like(log_psi) }, // =grad_outputs
        /*retain_graph=*/true, // =retain_graph
        /*create_graph=*/true  // =create_graph, needed so we can differentiate again
    )[0];
...
std::vector<double> NN_envelope::paramGradientLnAbs(std::vector<std::unique_ptr<Particle>>& particles) {
    m_net.zero_grad();
    auto input = encode(particles); 
    auto log_psi = m_net.log_forward(input); 

    // Compute all parameter gradients in one pass
    log_psi.backward();
